<?php
/*
	Plugin Name: Custom Fullwidth Slides
...
// [current_url]
add_shortcode ('current_url', 'get_current_page_url');
...
//start the code
if( ! class_exists( 'avia_sc_slidecontent' ) )
{
	class avia_sc_slidecontent extends aviaShortcodeTemplate
	{
		/**
		...
		public function shortcode_insert_button()
		{
			...
			$this->config['name']					= __( 'Slide Content FW', 'avia_framework' );
			$this->config['tab']					= __( 'Content Elements', 'avia_framework' );
			$this->config['icon']					= AviaBuilder::$path['imagesURL'] . 'sc-slidecontent.png';
			$this->config['order']					= 30;
			$this->config['target']					= 'avia-target-insert';
			$this->config['shortcode']				= 'av_slidecontent';
//			$this->config['modal_data']				= array( 'modal_class' => 'flexscreen' );
			$this->config['tooltip']				= __( 'Display the content of slides (fullwidth)', 'avia_framework' );
...
//				for($i = 1; $i<101; $i++) $itemcount[$i] = $i;
			...
			$this->elements = array(
				...
				array(
					...
					array(
						'name'			=> __( 'Which Slide?', 'avia_framework' ),
						...
						'posttype'		=> array( 'portfolio', 'fwslide' ),
